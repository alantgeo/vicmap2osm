...
module.exports = (feature, rangeFeature, options) => {
  ...
  if (
    ...
    if (rangeParts.length === 2) {
      // U8 277-289 should be read as 227-289
      const firstPartSplitBySpace = rangeParts[0].split(' ')
      const firstPart = firstPartSplitBySpace[firstPartSplitBySpace.length - 1]
      const lastPart = rangeParts[1]

      if (firstPartSplitBySpace.length > 1) {
        console.log(`range ${rangeNumber} has space in first part`, rangeFeature)
        const type = rangeFeature?.properties['@type']
        const id = rangeFeature?.properties['@id']
        if (type && id) {
          console.log(`https://www.openstreetmap.org/${type}/${id}`)
        }
        console.log(`range ${rangeNumber} has space in first part`, rangeFeature)
      }

      const fromMatch = firstPart.match(regexp)
      const toMatch = lastPart.match(regexp)

      if (!fromMatch || !toMatch) {
        console.log(`range ${rangeNumber} didn't match regexp`, rangeFeature)
        return false
      }
      const from = firstPart.match(regexp).groups
      const to = lastPart.match(regexp).groups
