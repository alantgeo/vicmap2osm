...
i=0
totalFiles=`ls -1 dist/candidates-r2/newAddressesWithoutConflicts/*.osm | wc -l`
categoryMessage="New addresses"
stage="Stage 5"
DONE=dist/candidates-r2/newAddressesWithoutConflicts/_DONE
mkdir -p "$DONE"
for f in dist/candidates-r2/newAddressesWithoutConflicts/*.osm; do
    i=$(($i + 1))
    d=`dirname "$f"`
    b=`basename "$f" .osm`
